...
describe('PKService', () => {
  ...
  describe('listActive', () => {
    it('should return active PK matches', async () => {
      ...
      (db.select as jest.Mock).mockReturnValueOnce({
        from: leftJoinOrderedResult([mockMatch]).from,
      });
      // 2nd select: legions lookup by ids
      (db.select as jest.Mock).mockReturnValueOnce({
        from: pureWhere([{ id: 'legion-001', name: 'A', avatarUrl: null }]).from,
      });
  ...
  describe('findById', () => {
    it('should find by id', async () => {
      ...
      const whereFn = jest
        .fn()
        .mockResolvedValue([{ id: 'legion-001', name: '军团A', avatarUrl: null }]);
  ...
  describe('create', () => {
    ...
    it('should create', async () => {
      ...
      (db.select as jest.Mock).mockReturnValueOnce({
        from: singleResult({ legionId: 'legion-001' }).from,
      });
      (db.select as jest.Mock).mockReturnValueOnce({
        from: singleResult({ legionId: 'legion-002' }).from,
      });
      (db.select as jest.Mock).mockReturnValueOnce({
        from: singleResult({ membershipId: 'mem-001', role: 'leader' }).from,
      });
      // insert pk match with returning
      (db.insert as jest.Mock).mockReturnValueOnce(
        insertResult([{ pkId: 'pk-001', status: 'challenged' }]),
      );
  ...
  describe('vote', () => {
    ...
    it('should throw if not battling', async () => {
      (db.select as jest.Mock).mockReturnValueOnce({
        from: singleResult({ ...mockMatch, status: 'challenged' }).from,
      });
  ...
  describe('settle', () => {
    ...
    it('should handle tie', async () => {
      (db.select as jest.Mock).mockReturnValueOnce({
        from: singleResult({ ...mockMatch, scoreA: 5, scoreB: 5 }).from,
      });
