...
`.cursor/routes/` 是 skill/MCP/rule/hook 的**分类路由层**（按技术方向归类，资产保持扁平加载）：

- `README.md` — 总索引 + 快速决策表
- `01-frontend.md` ~ `07-project-context.md` — 7 个技术方向路由
- `08-mcp-servers.md` / `09-hooks.md` / `10-dormant.md` — MCP/Hooks/休眠资产
- `.cursor/rules/05-harness-routes.mdc`（alwaysApply）— 指引 Agent 做任务前查路由

资产不物理嵌套（Cursor 加载契约 + skills-lock 扁平路径），路由层做分类导航。
