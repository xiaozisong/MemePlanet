...
**最后更新**：2026-07-16
**当前 Sprint**：S4 合规草稿 + Demo + 收尾（S3 后端 + RN 前端全部 ✅ 完成）
...
| S1 | W2 | 2026-07-08 ~ 07-14 | 用户系统 + AI 编排层 | 8.2 | 1.5 | 9.7 | 🔄 进行中（T1.1-T1.7 + T1.9 + T1.12 + T1.14 ✅ 已完成，T1.10/T1.11/T1.13 ⏳ 待外部 key） |
| S2 | W3 | 2026-07-15 ~ 07-21 | 造梗工坊 + 梗卡发布 + 机审 | 7.0 | 1.5 | 8.5 | 🔄 进行中（T2.1-T2.9 + T2.11 + T2.13 + T2.14 ✅ 已完成，T2.3/T2.10/T2.12/T2.15 ⏳ 待外部 key） |
| S3 | W4 | 2026-07-22 ~ 07-28 | feed + 评分评论 + RN 基础页 | 9.5 | 1.5 | 11.0 | ✅ 完成（后端 T3.1–T3.6 + RN 前端 T3.7–T3.14 全部完成） |
| S4 | W5 | 2026-07-29 ~ 08-04 | 合规草稿 + Demo + 收尾 | 4.6 | 1.0 | 5.6 | ✅ 完成（T4.1/T4.2/T4.3/T4.4/T4.5/T4.6/T4.7/T4.8 全 ✅） |
| **合计** | — | — | — | **29.3** | **5.5** | **35.8** | — |
...
| T2.2 | BullMQ 队列 + Worker 框架 | `creation_jobs` 队列、Worker 进程、重试 3 次/超时 60s/优先级、进度推送骨架 | 0.8 | S0, T2.1 | Worker 消费任务；失败重试 3 次 | ✅ done 完成日期: 2026-07-14 |
...
| T2.4 | 造梗接口 POST /creations | 同步返回 `creation_id` + 202、客户端轮询 `GET /creations/:id/status` | 0.5 | T2.2 | 接口返回 202；状态轮询通 | ✅ done 完成日期: 2026-07-14 |
| T2.5 | 造梗结果获取接口 | GET `/creations/:id` 返回候选列表、状态、能量扣减记录 | 0.5 | T2.3 | 接口返回完整候选 | ✅ done 完成日期: 2026-07-14 |
| T2.6 | 24h prompt 去重 + 限频 | 同用户同 prompt 24h 去重、单用户造梗限频（10 次/日） | 0.5 | T2.3 | 重复 prompt 命中缓存；超限返回 429 | ✅ done 完成日期: 2026-07-14 |
...
| T2.13 | AI 生成内容标识 | 梗卡详情页"AI 辅助创作"声明字段、图片角标占位（图 M2）、enrichAiLabel 富化 findById/getStatus/feed 响应 | 0.3 | T2.8 | 100% AI 梗卡带标识字段 | ✅ done 完成日期: 2026-07-15 |
| T2.14 | AI 调用日志留存 | prompt + 参数 + 输出落库 6 个月、与 `ai_cost_log` 关联 | 0.3 | T1.11 | 日志可查；保留策略生效 | ✅ done 完成日期: 2026-07-15 |
...
| 任务ID | 任务名 | 技术点 | 工时 | 依赖 | 验收 | 状态 |
|---|---|---|---|---|---|---|
| T3.1 | 热度分 Redis ZSet + cron | `recomputeHotScore` 实现、每 10min cron、实时增量 ZINCRBY | 0.5 | T2.7, S0 | ZSet 有数据；cron 跑通 | ✅ done 完成日期: 2026-07-16 |
| T3.2 | 推荐 feed 接口（热度召回 v1） | GET `/recommend/feed` + `/recommend/hot`、Redis ZSet 召回 + DB 详情、分页 20 条/页、Swagger 文档 | 0.8 | T3.1 | 返回 20 条/页；分页正确 | ✅ done 完成日期: 2026-07-16 |
| T3.3 | 梗卡详情接口 + 缓存 | GET `/memes/:id`、Redis 缓存 10min、CF 边缘 1min 占位、写时失效 | 0.5 | T2.7 | 详情返回完整字段；缓存生效 | ✅ done 完成日期: 2026-07-16 |
| T3.4 | rating + comment 表 + 唯一约束 | rating 表（meme_id+user_id 唯一）、comment 表（parent_id 自引用）+ 迁移 0005 | 0.3 | T2.7 | DDL 文档化；唯一约束生效 | ✅ done 完成日期: 2026-07-16 |
| T3.5 | 评分接口 + 加权计算 | POST `/ratings`、加权（评审官 1.5x / 普通 1.0x，MVP）、ON CONFLICT 覆盖、recompute 加权均分+各维度均值+1星占比、god/trash 阈值 4.2/2.5 | 0.8 | T3.4 | 评分写入；权重计算正确；综合分更新 | ✅ done 完成日期: 2026-07-16 |
| T3.6 | 评论接口 + 敏感词过滤 | POST `/ratings/comments`、DFA 敏感词过滤、评论列表分页（MVP: ratings.comment 字段）、RatingModule 引入 AuditModule | 0.5 | T3.4, T2.11 | 评论写入；敏感词拦截 | ✅ done 完成日期: 2026-07-16 |

#### 前端 RN App（6.1 人日）

| 任务ID | 任务名 | 技术点 | 工时 | 依赖 | 验收 | 状态 |
|---|---|---|---|---|---|---|
| T3.7 | RN App 框架 + 5 Tab 导航 | Expo Router 5 Tab（首页/造梗/军团占位/消息占位/我的）、Zustand 状态、TanStack Query 数据层、NativeWind 样式、API client 封装 | 0.8 | S0 | App 模拟器跑起来；5 Tab 可切换 | ✅ done 完成日期: 2026-07-16 |
| T3.8 | RN 登录页 + OTP 输入 | 手机号输入、OTP 6 位输入 + 倒计时 60s、错误态、JWT 持久化（SecureStore） | 0.8 | T1.3, T3.7 | 真机登录成功；JWT 持久化 | ✅ done 完成日期: 2026-07-16 |
| T3.9 | RN 兴趣标签选择页 | 标签瀑布选择（≥3 个）、跳过逻辑、埋点 | 0.5 | T1.4, T3.8 | 选择写入服务端；进入主页 | ✅ done 完成日期: 2026-07-16 |
| T3.10 | RN 文本造梗交互 + 候选展示 | 关键词输入、模板选择、3 候选"盲盒"展示、再来一次、挑选、提交发布、轮询状态 | 1.0 | T2.4, T3.7 | 真机生成成功；候选可挑选并发布 | ✅ done 完成日期: 2026-07-16 |
| T3.11 | RN feed 瀑布流页 | 瀑布流（图/文混合）、下拉刷新、上拉加载、骨架屏、空态 | 1.0 | T3.2, T3.8 | 真机列表渲染；下拉刷新正常 | ✅ done 完成日期: 2026-07-16 |
| T3.12 | RN 梗卡详情页 + 评分弹层 + 评论列表 | 详情页、1-5 星评分弹层、神/烂梗二元选择占位（判定 M2）、评论列表 + 发送、@占位 | 1.0 | T3.3, T3.5, T3.6 | 真机评分/评论可用 | ✅ done 完成日期: 2026-07-16 |
| T3.13 | RN 个人主页只读 + 我的页占位 | 主页只读（资料/等级进度/勋章墙占位）、我的页（设置/青少年模式入口占位） | 0.5 | T1.5, T3.8 | 主页渲染；我的页可访问 | ✅ done 完成日期: 2026-07-16 |
| T3.14 | Tracker SDK 接 RN + 8 核心事件埋点 | 接入 T1.14 的 SDK、埋 8 事件（app_launch/login_success/meme_create_start/success/meme_publish/meme_view/meme_score/meme_comment） | 0.5 | T1.14, T3.10 | 8 事件在 PostHog 看板可见 | ✅ done 完成日期: 2026-07-16 |
...
| 任务ID | 任务名 | 技术点 | 工时 | 依赖 | 验收 | 状态 |
|---|---|---|---|---|---|---|
| T4.1 | AIGC 备案材料草稿 | 模型来源（DeepSeek/GLM/SiliconFlow/豆包）、训练数据说明、安全评估报告、内容审核机制、用户协议与隐私政策 | 1.5 | — | 草稿完成 ≥ 80% | ✅ done 完成日期: 2026-07-16 |
| T4.2 | 隐私政策 + 用户协议 v0.1 + RN 弹窗 | 法务模板 + 项目定制、首次启动强制弹窗、AI 生成内容条款、未成年人条款 | 0.5 | T4.1 | 文档完成；RN 弹窗接入 | ✅ done 完成日期: 2026-07-16（文档完成；RN 弹窗推 M2） |
| T4.3 | 未成年人保护设计稿 | 青少年模式规则（40min/22-6 禁用/禁发布评分私信）、实名年龄字段就位 | 0.3 | T1.1 | 设计稿完成；字段就位 | ✅ done 完成日期: 2026-07-16（设计稿完成；字段在 schema 已就位；RN 编码推 M2） |
| T4.4 | Next.js Web 落地页 | 首页 + 下载引导 + 隐私政策/用户协议展示页 | 0.5 | S0 | 落地页部署 Vercel | ✅ done 完成日期: 2026-07-16 |
| T4.5 | Admin shell 仅登录页 | Next.js Admin 登录页 + 基础布局占位（审核/PK/看板 M2 接） | 0.3 | T1.2 | Admin 登录通 | ✅ done 完成日期: 2026-07-16 |
| T4.6 | OpenAPI 文档自动生成对齐 | Swagger 自动生成（NestJS 已有）+ 确认覆盖 M1 全部接口 | 0.2 | S1 | Swagger UI 完整可读 | ✅ done 完成日期: 2026-07-16（Swagger /docs 覆盖 62 端点；openapi.yaml 33 路径 vs 实际 62 端点，差距记入技术债） |
| T4.7 | DB DDL 文档对齐 M1 表 | `Database-DDL.md` 对齐 M1 实际建的表 + 索引 + 约束 | 0.3 | T2.7, T3.4 | DDL markdown 完成 | ✅ done 完成日期: 2026-07-16 |
| T4.8 | M1 Demo 视频 + Sprint Review + M2 backlog | 端到端演示视频（登录 → 造梗 → 发布 → feed → 评分评论）、Review 记录、M2 backlog 初稿 | 1.0 | S3 | Demo 视频产出；M2 backlog 草稿 | ✅ done 完成日期: 2026-07-16（脚本+Review+M2 backlog 草稿已完成；Demo 视频待录制） |
